	...
	render(){
		return(<div id="signin">
			  <div className="signin">
				 ...
				 <input className="pass" type="password" placeholder="请输入您的密码" value={this.state.password} onChange={(e)=>{
					 this.setState({password:e.target.value})
				 }}/>
				 <button onClick={this.toToHome.bind(this)}>登录</button>
				 <p className="p2"><span onClick={this.register.bind(this)}>立即注册</span></p>
			  </div>	
    </div>)
	}
}